...
namespace KoiAI.Health
{
    public class HealthBarManager : MonoBehaviour
    {
        ...
        {
            if(health && !_dicHealthBar.ContainsKey(health))
            {
        ...
        ///  TryGet: 체력바 객체 반환
        /// </summary>
        private bool TryGetHealthBar(Health health, out HealthBar healthBar)
        {
        ...
        public HealthBar CreateOrGetHealthBar(Health health)
        {
            HealthData healthData = health.HealthData;
            bool bGet = TryGetHealthBar(health, out HealthBar healthBar);
            if (!bGet)
            {
                healthBar = CreateHealthBar(health);
            }
            healthBar.Init(health, healthData.HealthBarData, health.CurrentHealth, healthData.MaxHealth);
            return healthBar;
        }
